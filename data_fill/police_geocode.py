...
cursor = cnxn.cursor()
cursor.execute('SELECT [StationID], [address1], [city], [state], [zip], [lat], [lng] FROM PoliceStation WHERE [Lat] IS NULL OR [Lng] IS NULL;')
...
for row in cursor.fetchall():
    ...
    cursor.execute(upd_qry, [round(loc.latitude, 4), round(loc.longitude, 4), row.StationID])
